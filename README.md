...
│   └── INDEX-Attestra.json
├── attestra_core/                 # ★ 커널 — 단일 출처 결정론 verdict substrate (stdlib only) [예정]
│   ├── packet.py                  #   증거 패킷 모델 + private-payload 거부
...
├── attestra_packs/                # ★ 도메인 팩 — 각 HELIX 프로젝트의 predicate 계약 [예정]
│   └── handback/                  #   레퍼런스 팩 (ActionHandbackVerifier parity)
├── schemas/                       # JSON Schema (packet/verdict/ledger/attestation/manifest) [예정]
├── cli.py                         # sample/run/verify/report/attest/pack/ledger [예정]
└── tests/                         # 결정론 unittest (커널 + 팩 parity + 파이프라인) [예정]
```

> 현재 상태: **설계 완료(design phase)**. 코드 노드(`attestra_core/` 등)는 WORKPLAN Phase 1부터 구현 예정.
...
python cli.py run --pack handback --input examples/valid.json --ledger examples/ledger.jsonl
python cli.py attest --pack handback --input examples/valid.json     # warrant 발행
python cli.py verify --ledger examples/ledger.jsonl                  # 체인 무결성

# 다중 팩 합성 (SpendBoundary 일반화)
python cli.py run --pipeline context-boundary,spend-boundary,veto-escrow --input examples/spend.json

# ★ 닫힌 감사 루프: 디렉토리의 모든 패킷을 일괄 처리 → 단일 감사 원장 + attestation + 요약
python cli.py audit --dir examples --attest-out examples/attestations --report examples/audit.md
